...
function CodeVerification () {
	...
	const verifyCode = () => {
		authService
			.accountVerificationCode(email, verificationCode)
			.then((response) => {
				if (response.success) {
	...
	const resendCode = () => {
		authService
			...
			.then((response) => {
				if (response.success) {
					message.success("Account verification code sent. Check your email!!!");
				} else {
				...
				<div className="codeVerification_container_main">
					...
					<div className="codeVerification_container_main_textField">
						...
						<p style={fontStyles.smallRegular}>
							A conformation code has sent to email address
						</p>
					</div>
